    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.contacts.update", [$contact->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('phone_number') ? 'is-invalid' : '' }}" type="number" name="phone_number" id="phone_number" value="{{ old('phone_number', $contact->phone_number) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('whatsapp_number') ? 'is-invalid' : '' }}" type="number" name="whatsapp_number" id="whatsapp_number" value="{{ old('whatsapp_number', $contact->whatsapp_number) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('organization') ? 'is-invalid' : '' }}" name="organization" id="organization">
                    ...
                    @foreach(App\Models\Contact::ORGANIZATION_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('organization', $contact->organization) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
